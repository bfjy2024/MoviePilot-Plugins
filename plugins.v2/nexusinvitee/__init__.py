...
class Prescription():
    ...
    def setInviteReason(self,site_name,value):
        self._tag(site_name,"invite_reason",value)
    ...
    def setSiteUrl(self, site_name, value):
        self._tag(site_name, "site_url", value or "")

    def setUserId(self, site_name, value):
        self._tag(site_name, "user_id", value or "")

    def setInviteeCount(self, site_name, value):
        self._tag(site_name, "invitees", value)

    def setDeadCount(self, site_name, value):
        self._tag(site_name, "dead", value)
    ...
    def _export(self):
        ...
        for k in self._cache:
            ...
            # 如果有错误则记录到失败列表，并跳过加入药单详情
            if site_error:
                failed_list.append({
                    "site": site_name,
                    "site_url": self._cache[k].get("site_url", ""),
                    "remain": 0,
                    "can_buy": 0,
                    "invitees": 0,
                    "dead": 0,
            ...
            if site_remain + site_can_buy > 0:
                site_content = {
                    "site": site_name,
                    "site_url": self._cache[k].get("site_url", ""),
                    "user_id": self._cache[k].get("user_id", ""),
                    "remain": site_remain,
                    "can_buy": site_can_buy,
                    "invitees": self._cache[k].get("invitees", 0),
                    "dead": self._cache[k].get("dead", 0),
                    "can_invite": self._cache[k].get("can_invite", False),
                    "invite_reason": self._cache[k].get("invite_reason", ""),
                    "error": ""
                }
        ...
        # 返回时将成功与失败分开，方便界面并列展示且药单详情不包含失败项
        return {
            "total": {
                "remain": total_remain,
            ...
            "details": sorted(
                med_list,
                key=lambda x: (
                    not x.get('can_invite', False),
                    -x['remain'],
                    -x['can_buy'],
                    x['site']
                )
            ),
            "failed": sorted(failed_list, key=lambda x: x['site'])
        }
    ...
    def getExportText(self):
        ...
        # 仅导出药单详情（不包含错误站点）
        # 仅导出药单详情（不包含错误站点）
        for site in med_data.get("details", []):
            if site.get("can_invite"):
                can_invite_status = "YES"
            else:
                reason = site.get("invite_reason") or "不可邀请"
                can_invite_status = f"{reason}"
            med_text += f"站点[{site['site']}]: 剩余[{site['remain']}]个. 可购买[{site['can_buy']}]个. 后宫数[{site.get('invitees', 0)}]({site.get('dead', 0)})个. {can_invite_status}\r\n"
        return med_text

    def getComponent(self):
        med_data = self._export()
        # 如果没有任何药单详情且没有失败项，直接返回空
        if not med_data.get("details") and not med_data.get("failed"):
            return None

        # 生成药单内容字符串 - 仅包含成功项
        med_text = self.getExportText()
        failed_sites = med_data.get("failed", [])

        # 生成便于粘贴到 Excel 的制表符分隔文本（不含首行标题），每行: 站点\t剩余\t可购买\t发药权限
        tsv_lines = []
        for s in med_data.get('details', []):
            can_invite_status = "YES" if s.get("can_invite") else (s.get("invite_reason") or "不可邀请")
            tsv_lines.append(f"{s.get('site','')}\t{s.get('remain',0)}\t{s.get('can_buy',0)}\t{s.get('invitees',0)}({s.get('dead',0)})\t{can_invite_status}")
        tsv_text = "\n".join(tsv_lines)
        # 使用 json.dumps 来安全地将 Python 字符串嵌入 JS 字符串
        js_safe_med_text = json.dumps(tsv_text)
            ...
            "content": [
                ...
                },
                {
                    ...
                    "content": [
                        ...
                        # 垂直堆叠：先显示药单详情，再显示失败站点详情（纵向并列）
                        {
                            "component": "style",
                            "content": ".nexus-drug-table th, .nexus-drug-table td {text-align:center; vertical-align: middle;} .nexus-drug-table thead {background:#f5f7fa; font-weight:600;} .nexus-drug-table tbody tr:nth-child(odd){background:#fbfbfc;} .nexus-drug-table td{padding:8px 12px;}"
                        },
                        {
                            ...
                            "content": [
                                {
                                    ...
                                    "content": [
                                        {"component": "VExpansionPanelTitle", "text": f"药单详情 ({len(med_data.get('details', []))})"},
                                        {
                                            ...
                                            "content": [
                                                    {
                                                    "component": "VTable",
                                                    "props": {"density": "compact", "hover": True, "class": "nexus-drug-table text-caption"},
                                                    "content": [
                                                        {"component": "thead", "content": [{"component": "tr", "content": [
                                                            {"component": "th", "text": "站点"},
                                                            {"component": "th", "text": "现药"},
                                                            {"component": "th", "text": "可购买"},
                                                            {"component": "th", "text": "发药权限"},
                                                            {"component": "th", "text": "后宫数(死亡数)"}
                                                        ]}]},
                                                        {"component": "tbody", "content": [
                                                            {"component": "tr", "content": [
                                                                {"component": "td", "content": [{"component": "a", "props": {"href": s.get("site_url", ""), "target": "_blank", "rel": "noopener noreferrer", "style": "color: #1976D2; text-decoration: none;"}, "text": s["site"]}]} if s.get("site_url") else {"component": "td", "text": s["site"]},
                                                                {"component": "td", "content": [{"component": "a", "props": {"href": s.get("site_url") and (urljoin(s.get("site_url", ""), "invite.php") + ("?id=" + str(s.get("user_id", "")) if s.get("user_id") else "")) or "", "target": "_blank", "rel": "noopener noreferrer", "style": "color: #388E3C; text-decoration: none; font-weight: 500;"}, "text": str(s["remain"])}]} if s.get("site_url") else {"component": "td", "text": str(s["remain"])},
                                                                {"component": "td", "content": [{"component": "a", "props": {"href": s.get("site_url") and urljoin(s.get("site_url", ""), "mybonus.php") or "", "target": "_blank", "rel": "noopener noreferrer", "style": "color: #F57C00; text-decoration: none; font-weight: 500;"}, "text": str(s["can_buy"]) }]} if s.get("site_url") else {"component": "td", "text": str(s["can_buy"])},
                                                                {"component": "td", "props": {"class": ("success--text" if s.get("can_invite") else "error--text")}, "text": "✓ YES" if s.get("can_invite") else (s.get("invite_reason") or "✗ 不可邀请")},
                                                                {"component": "td", "text": f"{s.get('invitees', 0)}({s.get('dead', 0)})"}
                                                            ]} for s in med_data.get('details', [])
                                                        ]}
                                                    ]
                                                }
                                ...
                                },
                                {
                                    "component": "VExpansionPanel",
                                    "content": [
                                        {"component": "VExpansionPanelTitle", "text": f"获取信息失败站点 ({len(failed_sites)})"},
                                        {
                                            "component": "VExpansionPanelText",
                                            "props": {"class": "pa-0"},
                                            "content": [
                                                    ...
                                                    "content": [
                                                        ...
                                                        {"component": "tbody", "content": [
                                                            {"component": "tr", "content": [
                                                                {"component": "td", "content": [{"component": "a", "props": {"href": f.get("site_url", ""), "target": "_blank", "rel": "noopener noreferrer", "style": "color: #1976D2; text-decoration: none;"}, "text": f["site"]}]} if f.get("site_url") else {"component": "td", "text": f["site"]},
                                                                {"component": "td", "text": f.get("error")}
...
class nexusinvitees(_PluginBase):
    # 插件名称
    plugin_name = "后宫管理系统(自改版)"
    # 插件描述
    ...
    # 插件作者
    plugin_author = "madrays,bfjy"
    # 作者主页
    author_url = "https://bfjy2024.github.io/bfjy"
    # 插件配置项ID前缀
    plugin_config_prefix = "nexusinvitees_"
    # 加载顺序
    plugin_order = 21
    # 可使用的用户级别
    auth_level = 2
    ...
    def __reload_modules(self):
        ...
        try:
            ...
            for module_name in list(sys.modules.keys()):
                if module_name.startswith('plugins.nexusinvitees.') and module_name != 'plugins.nexusinvitees':
            ...
            importlib.import_module('plugins.nexusinvitees.data')
            importlib.import_module('plugins.nexusinvitees.utils')
            importlib.import_module('plugins.nexusinvitees.module_loader')
            ...
            try:
                from plugins.nexusinvitees.data import DataManager
                from plugins.nexusinvitees.utils import NotificationHelper
                from plugins.nexusinvitees.module_loader import ModuleLoader
    ...
    def get_dashboard(self, key: str, **kwargs) -> Optional[Tuple[Dict[str, Any], Dict[str, Any], List[dict]]]:
        ...
        获取插件仪表盘页面
        """
        if key != "nexusinvitees_dashboard":
            return None
...
"content": [
    ...
    def get_page(self) -> List[dict]:
        ...
        try:
            ...
            for site_name, cache in cached_data.items():
                ...
                total_temp_invites += invite_status.get("temporary_count", 0)
                # 保存站点 URL 以便后续渲染超链接
                self.presc.setSiteUrl(site_name, site_url_map.get(site_name, ""))
                # 保存用户ID以便后续生成邀请链接
                user_id = site_cache_data.get("user_id", "") or (site_cache_data.get("data", {}).get("user_id", "") if "data" in site_cache_data else "")
                self.presc.setUserId(site_name, user_id)
                self.presc.setInviteeCount(site_name, len(invitees))
                self.presc.setDeadCount(site_name, banned_count)
                # 向药单打标临药永药
            ...
            for site_name, cache in cached_data.items():
                ...
                if site_info:
                    ...
                    self.presc.setInviteReason(site_name, reason)
    ...
    def _get_site_invite_data(self, site_name):
        ...
        try:
            ...
            if is_mteam:
                ...
                from plugins.nexusinvitees.sites.mteam import MTeamHandler
                handler = MTeamHandler()
            elif "hdchina" in site_url.lower():
                logger.info(f"站点 {site_name} 使用HDChina处理器")
                from plugins.nexusinvitees.sites.hdchina import HDChinaHandler
                ...
                if not handler:
                    ...
                    from plugins.nexusinvitees.sites.nexusphp import NexusPhpHandler
            ...
            if user_id:
                ...
                logger.debug(f"站点 {site_name} 获取到用户ID: {user_id}")
            else:
                logger.warning(f"站点 {site_name} 无法获取用户ID")
    ...
    def _safe_base64_decode(self, value: str) -> str:
        """尝试对 Base64 字符串进行解码，失败时返回空字符串。"""
        try:
    ...
    def _extract_user_id_from_cookie(self, cookie_header: str) -> str:
        """从 Cookie 值中尝试 Base64 解码并提取 user_id。"""
        ...
        for cookie_pair in cookie_header.split(";"):
            ...
            decoded = self._safe_base64_decode(value)
            if not decoded:
                continue

            for pattern in [r'userdetails\.php\?id=(\d+)', r'userid=(\d+)', r'id=(\d+)', r'uid=(\d+)']:
                matches = re.search(pattern, decoded)
                if matches:
                    return matches.group(1)

            if re.fullmatch(r"\d{4,}", decoded.strip()):
    ...
    def _get_user_id(self, session: requests.Session, site_info: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # 先尝试从 Cookie 中 Base64 解码获取 user_id
            site_cookie = site_info.get("cookie", "").strip()
            if site_cookie:
                cookie_user_id = self._extract_user_id_from_cookie(site_cookie)
                if cookie_user_id:
                    logger.debug(f"从 Cookie Base64 解码获取到用户ID: {cookie_user_id}")
    ...
    def get_api_handlers():
        ...
        """
        return {
            "/get_invitees": {"func": nexusinvitees.get_invitees, "methods": ["GET"], "desc": "获取所有站点邀请数据"},
            "/refresh": {"func": nexusinvitees.refresh_data, "methods": ["GET"], "desc": "强制刷新站点数据"}
        }
