...
// Utilities
const {camelize,capitalize,Comment,Fragment,isProxy,isReactive,isRef,isVNode,reactive: reactive$3,shallowRef: shallowRef$5,toRaw: toRaw$1,toRef: toRef$6,unref: unref$1,watchEffect: watchEffect$3} = await importShared('vue');
...
function deepToRaw(value) {
  const objectIterator = input => {
    ...
    if (isRef(input) || isReactive(input) || isProxy(input)) {
      return objectIterator(toRaw$1(input));
    }
...
const {computed: computed$5,inject: inject$6,onActivated,onBeforeUnmount,onDeactivated,onMounted,provide: provide$2,reactive: reactive$2,ref: ref$4,shallowRef: shallowRef$3,toRef: toRef$5,useId} = await importShared('vue');
...
function createLayout(props) {
  ...
  provide$2(VuetifyLayoutKey, {
    register: (vm, _ref2) => {
      ...
      const layoutItemStyles = computed$5(() => {
      ...
      const layoutItemScrimStyles = computed$5(() => ({
  ...
  const layoutClasses = toRef$5(() => ['v-layout', {
    'v-layout--full-height': props.fullHeight
  }]);
  const layoutStyles = toRef$5(() => ({
...
const {computed: computed$4,ref: ref$3,toRaw,watch: watch$3} = await importShared('vue');
// Composables
function useProxiedModel(props, prop, defaultValue) {
  ...
  const isControlled = checkKebab ? computed$4(() => {
    void props[prop];
...
const {ref: ref$2,shallowRef: shallowRef$2,toRef: toRef$4,watch: watch$2} = await importShared('vue');
...
// Utilities
const {computed: computed$2,getCurrentScope,inject: inject$4,onScopeDispose: onScopeDispose$1,provide,ref,shallowRef: shallowRef$1,toRef: toRef$2,watch: watch$1,watchEffect: watchEffect$1} = await importShared('vue');
...
function createTheme(options) {
  ...
  const themeClasses = toRef$2(() => parsedOptions.isDisabled ? undefined : `${parsedOptions.prefix}theme--${name.value}`);
...
function provideTheme(props) {
  ...
  const themeClasses = toRef$2(() => theme.isDisabled ? undefined : `${theme.prefix}theme--${name.value}`);
  const newTheme = {
    ...theme,
    name,
    current,
...
// Utilities
const {computed: computed$1,inject: inject$3,toValue,createVNode:_createVNode,createElementVNode:_createElementVNode,mergeProps:_mergeProps,normalizeClass:_normalizeClass} = await importShared('vue');
...
function splitKeyCombination(combination) {
  ...
  const hasInvalidLeadingSeparator = combination.length > 1 &&
  ...
  const hasInvalidStructure = hasInvalidLeadingSeparator ||
